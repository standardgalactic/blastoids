...
\section{The Spherepop Language: Operational Semantics}

We now formalize the generative operator $\mathcal{G}$ as a computational language. The Spherepop language is defined as an event-driven, history-preserving system in which all computation is realized as irreversible transformations of the KES state $\mathfrak{S}_t$.

\subsection{State and Evaluation}

A program state is identified with the KES tuple $\mathfrak{S}_t = (X_t, \Omega_t, H_t, \Psi_t, \mathcal{M}_t)$. Evaluation is defined as a transition
\[
\mathcal{G} : (\mathfrak{S}_t, e) \mapsto \mathfrak{S}_{t+1},
\]
where $e \in \Sigma$ is a syntactic event.

\begin{proposition}
Evaluation is strictly monotonic in history:
\[
H_{t+1} = H_t \cdot e,
\]
where $\cdot$ denotes ordered concatenation.
\end{proposition}

\subsection{Scope Geometry}

A Spherepop program consists of nested regions, or \emph{scopes}, defined as submanifolds of $X_t$ equipped with local histories.

\begin{definition}[Scope]
A scope $\sigma$ is a tuple $\sigma = (X_\sigma, H_\sigma)$, where $X_\sigma \subseteq X_t$ and $H_\sigma \subseteq H_t$ is the restriction of history relevant to that region.
\end{definition}

Scopes form a partially ordered structure under inclusion: $\sigma_1 \preceq \sigma_2 \iff X_{\sigma_1} \subseteq X_{\sigma_2}$.

\begin{remark}
Scopes are projections, not containers: they restrict observational access to $H_t$ rather than isolating state.
\end{remark}

\subsection{Merge Operator}

\begin{definition}[Merge]
Given scopes $\sigma_a = (X_a, H_a)$ and $\sigma_b = (X_b, H_b)$, define
\[
\sigma_a \cup \sigma_b = (X_a \cup X_b,\; H_a \sqcup H_b),
\]
where $H_a \sqcup H_b = \{(h, \ell) \mid h \in H_a \cup H_b,\; \ell \in \{a,b\}\}$ is the provenance-preserving union.
\end{definition}

\begin{proposition}
Merge is associative but not strictly commutative: $(\sigma_a \cup \sigma_b) \cup \sigma_c \cong \sigma_a \cup (\sigma_b \cup \sigma_c)$, but provenance labels in $H_t$ retain ordering information.
\end{proposition}

\begin{remark}
Merge does not erase differences. It constructs a joint space in which conflicting histories remain distinguishable.
\end{remark}

\subsection{Collapse Operator}

\begin{definition}[Collapse]
Let $\sigma = (X_\sigma, H_\sigma)$. The collapse operator is
\[
\downarrow : \sigma \to \tilde{\sigma},
\]
where $\tilde{\sigma}$ is a reduced representation satisfying $\mathcal{M}_t(H_\sigma) = \mathcal{M}_t(H_{\tilde{\sigma}})$.
\end{definition}

\begin{proposition}
Collapse is information-reducing but valuation-preserving:
\[
\exists f : H_\sigma \twoheadrightarrow H_{\tilde{\sigma}} \quad \text{such that}\quad \mathcal{M}_t \circ f = \mathcal{M}_t.
\]
\end{proposition}

\begin{remark}
Collapse corresponds to abstraction. It constructs a quotient representation compatible with $\mathcal{M}_t$ without deleting history; full provenance from representation back to $H_t$ is preserved.
\end{remark}

\subsection{Append Kernel}

\begin{definition}[Append]
For an event $e$, define $\ll : (H_t, e) \mapsto H_{t+1} = H_t \cdot e$.
\end{definition}

\begin{proposition}
The append operation admits no inverse: $\not\exists\; \ll^{-1}$.
\end{proposition}

\begin{remark}
This enforces log sovereignty at the computational level: computation is identical with irreversible inscription.
\end{remark}

\subsection{Evaluation Loop and Universality}

The execution of a Spherepop program is given by iterative application of $\mathfrak{S}_{t+1} = \mathcal{G}(\mathfrak{S}_t, e_t)$ with $e_t \in \{\cup, \downarrow, \ll\}$.

\begin{proposition}
All computable transformations reduce to compositions of merge, collapse, and append. The grammar is computationally universal provided $H_t$ supports recursive self-reference through normal-form fixed points.
\end{proposition}

\begin{remark}
Spherepop is therefore the minimal irreversible calculus: all higher-order constructs are derived operations over these three primitives. It is not a simulation of the KES framework but its direct computational instantiation.
\end{remark}

\section{A Dependent Type System for Spherepop}

We endow Spherepop with a dependent type system whose purpose is preservation of provenance, irreversibility, and scope coherence. Types track not only the shape of values but the historical conditions under which those values are valid.

\subsection{Judgments and Core Types}

Typing judgments take the form
\[
\Gamma;\, H \vdash e : A,
\]
where $\Gamma$ is a scope-sensitive context, $H$ is the current event history, $e$ is an expression, and $A$ is its type. A term is not typed absolutely but relative to a history, making the type system non-Markovian at the judgmental level. We also use
\[
\Gamma;\, H \vdash e \leadsto e' : A
\]
for evaluation that produces $e'$ while extending the history.

The primitive types are $\mathbf{Scope}$, $\mathbf{Event}$, $\mathbf{Hist}$, $\mathbf{Val}$, $\mathbf{Trace}$, together with dependent formers $\Pi_{x:A} B(x)$ and $\Sigma_{x:A} B(x)$, an indexed history type $\mathbf{Hist}(n)$ whose inhabitants are histories of length $n$, a history-sensitive scope type $\mathbf{Scope}(H)$, and $\mathbf{Event}(H)$ for events admissible at history $H$.

\subsection{Typing of Merge}

\[
\cup\; :\; \Pi_{H_a : \mathbf{Hist}}\, \Pi_{H_b : \mathbf{Hist}}\; \mathbf{Scope}(H_a) \to \mathbf{Scope}(H_b) \to \mathbf{Scope}(H_a \sqcup H_b).
\]

Typing rule:
\[
\frac{
\Gamma;\, H \vdash \sigma_a : \mathbf{Scope}(H_a) \qquad \Gamma;\, H \vdash \sigma_b : \mathbf{Scope}(H_b)
}{
\Gamma;\, H \vdash \sigma_a \cup \sigma_b : \mathbf{Scope}(H_a \sqcup H_b)
}
\]

\subsection{Typing of Collapse}

\[
\downarrow\; :\; \Pi_{H : \mathbf{Hist}}\; \mathbf{Scope}(H) \to \Sigma_{\tilde H : \mathbf{Hist}}\, \mathbf{Trace}(\tilde H),
\]
subject to $\mathcal{M}(H) = \mathcal{M}(\tilde H)$.

Typing rule:
\[
\frac{
\Gamma;\, H \vdash \sigma : \mathbf{Scope}(H_\sigma)
}{
\Gamma;\, H \vdash \downarrow \sigma : \Sigma_{\tilde H : \mathbf{Hist}}\, \mathbf{Trace}(\tilde H)
}
\]

\subsection{Typing of Append}

\[
\ll\; :\; \Pi_{n : \mathbb{N}}\; \mathbf{Hist}(n) \to \mathbf{Event}(\mathbf{Hist}(n)) \to \mathbf{Hist}(n+1).
\]

This makes irreversibility a typing fact rather than a runtime convention.

\subsection{Log Sovereignty in the Type Theory}

Refine append with a derivation witness:
\[
\ll\; :\; \Pi_{n:\mathbb{N}}\, \Pi_{h:\mathbf{Hist}(n)}\, \Pi_{e:\mathbf{Event}(h)}\; \mathrm{Derives}(h,e) \to \mathbf{Hist}(n+1),
\]
where $\mathrm{Derives}(h,e) : \mathbf{Prop}$ certifies causal provenance. No event may be appended without a derivation witness; log sovereignty is internal to the type system.

\subsection{Preservation and Progress}

\begin{theorem}[Type Preservation]
If $\Gamma;\, H \vdash e : A$ and $e \leadsto e'$, then there exists $H' \sqsupseteq H$ such that $\Gamma;\, H' \vdash e' : A$.
\end{theorem}

\begin{proof}[Sketch]
By induction on evaluation. Merge preserves scope typing by construction of $\mathbf{Scope}(H_a \sqcup H_b)$. Collapse preserves trace typing by quotient compatibility with $\mathcal{M}$. Append extends history length by one while preserving validity via the derivation witness.
\end{proof}

\begin{theorem}[Irreversible Progress]
If $\Gamma;\, H \vdash e : A$, then either $e$ is canonical or there exist $e'$ and $H' \sqsupset H$ such that $\Gamma;\, H' \vdash e \leadsto e' : A$.
\end{theorem}

\begin{remark}
Progress is history-extending rather than state-preserving, reflecting the irreversible character of computation in this framework.
\end{remark}

\subsection{Identity and Equality}

Two notions of equality arise. Judgmental equality $e \equiv e'$ captures canonical equivalence. Historical equality $e \sim_H e'$ holds if $e$ and $e'$ induce equivalent traces in $H$. Two expressions may collapse to the same trace without being judgmentally identical, preserving the distinction between abstraction and erasure.

\section{Categorical Semantics of Spherepop}

We provide semantics for Spherepop in categorical terms, showing that the operational system is an instance of the broader KES ontology.

\subsection{Base Category of Histories}

Let $\mathcal{H}$ be a category whose objects are histories and whose morphisms are history extensions: $H \to H'$ iff $H$ is a prefix of $H'$. Since histories are append-only, $\mathcal{H}$ is a preorder-enriched category.

\subsection{Fibered Category of Scopes}

Let $\pi : \mathcal{S} \to \mathcal{H}$ be a fibration, where the fiber over $H$ is the category of scopes valid relative to $H$: $\mathcal{S}_H = \pi^{-1}(H)$. Merge is interpreted fiberwise as a lax monoidal operation
\[
\otimes_H : \mathcal{S}_{H_a} \times \mathcal{S}_{H_b} \to \mathcal{S}_{H_a \sqcup H_b},
\]
lax because provenance is preserved rather than collapsed.

\subsection{Append and Collapse as Functors}

An event $e : H \to H'$ induces a pushforward $e_* : \mathcal{S}_H \to \mathcal{S}_{H'}$; the operational append corresponds to $e_*$. Collapse is a reflector $Q_H : \mathcal{S}_H \to \mathcal{T}_H$ with adjunction $Q_H \dashv I_H$, where $I_H : \mathcal{T}_H \hookrightarrow \mathcal{S}_H$ is the inclusion of canonical traces.

\subsection{Log Sovereignty as Semantic Commutativity}

Log sovereignty becomes the requirement that the semantic square commute:
\[
\begin{tikzcd}
\mathcal{S}_H \arrow[r, "e_*"] \arrow[d, "Q_H"'] & \mathcal{S}_{H'} \arrow[d, "Q_{H'}"] \\
\mathcal{T}_H \arrow[r, "\tilde{e}_*"] & \mathcal{T}_{H'}
\end{tikzcd}
\]
No projection may rewrite history retroactively.

\subsection{KES Correspondence}

The KES state $\mathfrak{S}_t$ is interpreted as: $H_t$ an object of $\mathcal{H}$; $\Omega_t \subseteq \mathcal{S}_{H_t}$ a structured fiber; $\mathcal{G}$ an endofunctor on the fibred system compatible with base extension; and $\mathcal{M}_t$ factoring through traces:
\[
\mathcal{S}_{H_t} \xrightarrow{Q_{H_t}} \mathcal{T}_{H_t} \xrightarrow{\mathcal{M}_t} \mathbb{R}.
\]
Valuation occurs only after collapse, which formally explains why projection induces structural loss.

\begin{remark}
Spherepop is a fibred, history-indexed calculus in which computation is the transport of structured possibility into irreversible event space. In the $(\infty,1)$-categorical lift, merge is monoidal composition, collapse is reflection, and append is directed base change.
\end{remark}

\section{Typed Fixed Points and Causal Self-Reference}

We extend Spherepop with recursion compatible with append-only history, replacing classical fixed points with history-indexed causal fixed points.

\subsection{Causal Fixed Points}

\begin{definition}[Causal Fixed Point]
Let $F : \Pi_{H:\mathbf{Hist}} A(H) \to A(H \cdot e)$. A causal fixed point of $F$ is a family $x : \Pi_{H:\mathbf{Hist}} A(H)$ satisfying $x(H \cdot e) = F(H, x(H))$ for all $H$.
\end{definition}

\begin{remark}
Causal fixed points are non-Markovian: the value at $H \cdot e$ depends on the entire preceding history through $x(H)$.
\end{remark}

\subsection{Recursion Operator and Guardedness}

The recursion operator $\mathbf{fix}$ has type
\[
\mathbf{fix} : \Pi_{A:\mathbf{Hist}\to\mathbf{Type}} \left(\Pi_{H:\mathbf{Hist}} A(H) \to A(H \cdot e)\right) \to \Pi_{H:\mathbf{Hist}} A(H),
\]
with defining equation $\mathbf{fix}(F)(H \cdot e) = F(H, \mathbf{fix}(F)(H))$.

\begin{definition}[Guarded Operator]
$F$ is guarded if every recursive occurrence of its argument appears only within an append: $F(H,x) = G(H, x(H))$ only within $H \cdot e$.
\end{definition}

\begin{proposition}
If $F$ is guarded, then $\mathbf{fix}(F)$ defines a unique causal fixed point. Recursion produces new history at each step; there are no instantaneous self-referential loops.
\end{proposition}

\subsection{No Global Fixed Points}

\begin{theorem}
There exists no global fixed point $x$ satisfying $x = F(x)$ independently of history.
\end{theorem}

\begin{proof}
Any such equality would imply $x(H) = x(H \cdot e)$, contradicting strict monotonicity of $H$ under append.
\end{proof}

\begin{remark}
This eliminates classical paradoxes of self-reference. Fixed points exist only as evolving trajectories, never as static objects.
\end{remark}

\subsection{Trace Fixed Points and Identity}

Two distinct causal fixed points may collapse to the same trace fixed point $\tilde{x}$ satisfying $Q(x(H \cdot e)) = Q(F(H, x(H)))$. This formalizes the distinction between behavioral equivalence and historical identity.

\begin{definition}[Identity as Trajectory]
The identity of a term is the equivalence class of its causal fixed point trajectory: $\mathrm{Id}(x) = \{x(H) \mid H \in \mathbf{Hist}\}$.
\end{definition}

\begin{proposition}
Identity is invariant under reparameterization of history but not under collapse.
\end{proposition}

\subsection{Categorical Interpretation}

In the categorical semantics, causal fixed points correspond to coalgebras of the history-shift endofunctor $T(A)(H) = A(H \cdot e)$. A causal fixed point is a coalgebra $x : A \to T(A)$ unfolding along the morphisms of $\mathcal{H}$. Recursion in Spherepop is therefore coinductive: computation is unfolding, not resolution.

\section{Geometry of Causal Fixed Point Trajectories}

We introduce a geometric structure on the space of causal trajectories, measuring how history-indexed unfoldings diverge, converge, or remain equivalent under projection.

\subsection{Trajectory Space and Divergence}

The space of causal trajectories of type family $A$ is
\[
\mathfrak{T}_A = \{x \mid x : \Pi_{H:\mathbf{Hist}} A(H)\}.
\]
Let $\mu_H^x$ and $\mu_H^y$ be measures induced by trajectories $x, y$ over admissible future extensions at history $H$. The history-indexed KL divergence is
\[
D_{\mathrm{KL}}^H(x \Vert y) = \sum_{\omega \in \Omega_H} \mu_H^x(\omega) \log \frac{\mu_H^x(\omega)}{\mu_H^y(\omega)}.
\]

\begin{definition}[Trajectory Divergence]
The divergence of $x$ from $y$ over history horizon $\mathcal{H}$ is
\[
D_{\mathcal{H}}(x \Vert y) = \sum_{H \in \mathcal{H}} w(H)\, D_{\mathrm{KL}}^H(x \Vert y),
\]
for weights $w(H) \geq 0$.
\end{definition}

\subsection{Causal Fisher Metric}

For trajectories parameterized by $\theta \in \Theta$, define the local Fisher information tensor
\[
g_{ij}(H,\theta) = \sum_{\omega \in \Omega_H} \mu_H^\theta(\omega)\, \partial_i \log \mu_H^\theta(\omega)\, \partial_j \log \mu_H^\theta(\omega),
\]
and the causal Fisher metric
\[
G_{ij}(\theta) = \sum_{H \in \mathcal{H}} w(H)\, g_{ij}(H,\theta).
\]

\begin{proposition}
$G_{ij}$ defines a Riemannian metric on $\Theta$ whenever the support of $\mu_H^\theta$ is constant and the family is locally identifiable.
\end{proposition}

\begin{remark}
This geometry is not a geometry of states but of history-indexed dispositions. Nearby points in $\Theta$ correspond to similar causal unfoldings.
\end{remark}

\subsection{Projection Equivalence and Historical Separation}

\begin{proposition}
Projection equivalence ($Q_H(x(H)) = Q_H(y(H))$ for all $H$) does not imply zero trajectory divergence: the induced future measures $\mu_H^x$ and $\mu_H^y$ may still differ, yielding $D_{\mathrm{KL}}^H(x \Vert y) > 0$.
\end{proposition}

\section{Stability of Causal Fixed Points}

\subsection{Local Causal Stability}

\begin{definition}[Uniform Guarded Contraction]
$F$ is uniformly guarded-contracting on a cofinal history tail if there exists $\mathcal{H}_{\mathrm{tail}} \subseteq \mathbf{Hist}$ cofinal and $\lambda \in (0,1)$ with $|D_2 F(H, x^\ast(H))| \leq \lambda$ for all $H \in \mathcal{H}_{\mathrm{tail}}$.
\end{definition}

\begin{proposition}[Local Causal Stability]
If $F$ is uniformly guarded-contracting near $x^\ast$, then for all sufficiently small perturbations $\delta x$, $|\delta x(H \cdot e)| \leq \lambda |\delta x(H)|$ on the cofinal tail, so $x^\ast$ is locally causally stable.
\end{proposition}

\subsection{Causal Lyapunov Functionals}

\begin{definition}[Causal Lyapunov Functional]
A functional $V : \mathfrak{T}_A \times \mathbf{Hist} \to \mathbb{R}_{\geq 0}$ is a causal Lyapunov functional for $x^\ast$ if $V(x^\ast, H) = 0$, $V(x, H) > 0$ for $x \neq x^\ast$, and $V(x, H \cdot e) \leq V(x, H)$ along the evolution induced by $F$.
\end{definition}

\begin{proposition}
Existence of a causal Lyapunov functional implies stability of the corresponding fixed point trajectory.
\end{proposition}

\begin{proposition}
Historical stability implies projection stability, but not conversely: projection stability can hold while hidden historical structure remains distinct.
\end{proposition}

\section{Learning Rules for the Preference Field $\Psi_t$}

\subsection{Trajectory Mismatch and Natural Gradient}

Let $\mu_H^\Psi$ be the distribution over admissible events induced by preference field $\Psi$ at history $H$, and $\nu_H$ a target distribution. The mismatch functional is
\[
\mathcal{L}_{\mathcal{H}}(\theta) = \sum_{H \in \mathcal{H}} w(H)\, D_{\mathrm{KL}}(\nu_H \Vert \mu_H^\theta).
\]
The natural-gradient update is
\[
\theta_{t+1} = \theta_t - \eta_t\, G(\theta_t)^{-1} \nabla_\theta \mathcal{L}_{\mathcal{H}}(\theta_t).
\]

\begin{definition}[Care-Weighted Learning]
A learning rule is care-weighted if $w(H) = \alpha^{|H|} \chi(\mathcal{R}(H))$ with $\chi$ depending positively on structural richness, preventing the preference field from learning solely from high-frequency low-richness traces.
\end{definition}

\subsection{Online Learning from Append}

Given event $e_t$ appended at history $H_t$, the online update is
\[
\Psi_{t+1} = \Psi_t + \eta\Bigl(\phi(H_t, e_t) - \mathbb{E}_{\omega \sim \mu_{H_t}^{\Psi_t}}[\phi(H_t, \omega)]\Bigr),
\]
where $\phi(H,\omega)$ is a feature map over history-event pairs.

\begin{proposition}
This is an unbiased stochastic gradient step on the negative log-likelihood of realized trajectories.
\end{proposition}

\begin{remark}
Learning is not modification of a static parameter vector but curvature adjustment of a history-indexed field of possible transitions.
\end{remark}

\section{Asymptotic Stability Under Guarded Recursion and Care-Weighted Learning}

We now combine recursion, geometry, stability, and learning into a single theorem.

\subsection{Setup and Conditions}

Let $x^\ast = \mathbf{fix}(F)$ be a causal fixed point, and let the preference field $\Psi_t$ be parameterized by $\theta_t \in \Theta$ with natural-gradient updates under the care-weighted mismatch functional $\mathcal{L}_{\mathcal{H}}(\theta)$ with $w(H) = \alpha^{|H|}\chi(\mathcal{R}(H))$.

We assume: (i) $F$ is uniformly guarded-contracting on a cofinal history tail; (ii) the trajectory family is locally Fisher-regular near $\theta^\ast$; (iii) the target family $\nu_H$ is care-compatible, meaning $\chi(\mathcal{R}(H)) > 0$ and $\mathcal{L}_{\mathcal{H}}$ has a local minimum at $\theta^\ast$ preserving structural richness; (iv) Robbins--Monro conditions on learning rates: $\eta_t > 0$, $\sum_t \eta_t = \infty$, $\sum_t \eta_t^2 < \infty$.

\subsection{Main Theorem}

\begin{theorem}[Asymptotic Stability of Care-Weighted Causal Fixed Points]
Under conditions \textrm{(i)--(iv)}, there exists a neighborhood $U$ of $(x^\ast, \theta^\ast)$ such that for any initial condition in $U$:
\begin{enumerate}
\item the causal trajectory is historically stable;
\item the preference parameters converge asymptotically toward $\theta^\ast$;
\item the induced projected dynamics are projection-stable;
\item structural richness loss is bounded asymptotically by the care tolerance encoded in $\chi$.
\end{enumerate}
\end{theorem}

\begin{proof}[Sketch]
Guarded contraction implies local asymptotic stability of $x^\ast$ under history extension. Local Fisher regularity ensures the natural-gradient flow is well-defined near $\theta^\ast$. Robbins--Monro conditions imply asymptotic convergence to a local minimum of $\mathcal{L}_{\mathcal{H}}$, which by care-compatibility is $\theta^\ast$. Care weighting $\chi(\mathcal{R}(H))$ prevents the optimizer from improving mismatch by sacrificing all structurally rich histories, bounding richness degradation by the effective tolerance in the weighting scheme.
\end{proof}

\begin{corollary}[Projection-Level Alignment]
The collapsed traces $Q_H(x_t(H))$ converge locally toward the collapsed target dynamics induced by $\theta^\ast$.
\end{corollary}

\begin{corollary}[Anti-Goodhart Condition]
If $\chi(\mathcal{R}(H))$ is strictly increasing on the relevant structural regime, asymptotic optimization cannot collapse entirely onto low-richness proxy trajectories.
\end{corollary}

\begin{corollary}[Preference Drift Control]
Let $\Delta_t = D_{\mathcal{H}}(\theta_t \Vert \theta^\ast)$. Then $\Delta_t \to 0$ locally in the induced Fisher geometry.
\end{corollary}

\begin{remark}
A care-aligned KES system is not merely one that values the right things, but one whose recursive unfolding, adaptive deformation, and projection structure are jointly organized so that historically meaningful trajectories remain dynamically preferred. Guardedness prevents paradoxical self-reference, Fisher regularity supplies the geometry of adaptation, and care-weighting constrains optimization against pure proxy collapse.
\end{remark}

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Prerequisites: Generative Systems, Constraint, and the Path to This Framework}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The formal framework developed in this paper did not emerge from pure abstraction. It was built upward through a sequence of technical observations about generative systems, computational abstraction, and the structural differences between ecosystems and selves. This section reconstructs that sequence in objective terms, providing the conceptual prerequisites that the main theorems formalize.

\subsection{Computation as Layered Constraint}

The computational stack---binary representation, logic gates, boolean algebra, type systems, programming paradigms, user interfaces---is not a sequence of increasingly convenient notations. Each layer is a constrained projection of the layer below it. At the binary level, every operation is explicit and every constraint must be enforced directly. At the level of logic gates, sixteen fundamental boolean operations are realized from NAND or NOR primitives; the architecture constrains what transitions are physically possible. Type systems then restrict which programs can be written at all, enforcing invariants that would otherwise require explicit runtime checks. Programming paradigms---functional, object-oriented, declarative---reorganize constraints into patterns that prevent entire classes of error through structural rather than explicit means. The user interface presents only outputs: the constraint resolution that produced them is invisible.

This observation generalizes far beyond software engineering. Every stable interface over a computational substrate exists because a constraint system below it enforces invariants that prevent incoherent states from reaching the surface. The user sees a coherent result not because the result was produced coherently at all levels simultaneously, but because each intermediate layer filtered out the incoherence native to its substrate. This is the principle that the Abstraction Theorem later formalizes: a stable interface exists if and only if a constraint functional enforces global coherence at each layer.

\subsection{Text as Universal Substrate}

A natural question arising from this picture is whether any single representational medium could serve as a universal intermediate layer across all the others. The answer is yes, and the medium is text.

Text encoded in ASCII or Unicode is a universal encoding substrate in the following precise sense. Given any finite symbolic object---a program, a policy, a configuration, a mathematical structure---there exists an encoding of it as a string over a finite alphabet. This is not merely a statement about expressibility; it is a statement about the architecture of computation. Every executable behavior that can be specified can be specified in text, because text is the carrier of the symbolic instructions that interpreters, compilers, and models all consume.

The power of text as a universal substrate is that it enables chaining of transformations between heterogeneous systems. A pipeline of the form $X \to \text{text} \to Y$ uses text as a transport layer: the same symbolic object can be interpreted as a program by a compiler, as a policy by a decision system, and as a tool invocation by an orchestration layer. In each case the text is read by a different interpreter that extracts a different structure from the same string. This architecture is not novel---it underlies all of modern software---but its implications for the present framework are significant, because it means that the problem of maintaining coherent identity over time is formally equivalent to the problem of maintaining a coherent text that can be correctly interpreted by all the systems that need to act on it.

However, universality of representation does not imply universality of selection. The Library of Babel contains every possible string over its alphabet, and therefore every possible program, policy, and tool invocation. But nearly all of them are syntactically invalid, semantically incoherent, or operationally useless. The existence of a string in the library does not make it findable, executable, or meaningful. The hard problem is not representation---it is selection under constraint.

\subsection{The Markov Generator as Minimal Language Model}

A first-order Markov chain over characters, trained on a natural language corpus, is already a minimal language model in the precise technical sense. It defines a probability distribution over strings by factoring the joint probability as a product of conditional probabilities:
\[
P(w) = \prod_{t=1}^{n} P(\ell_t \mid \ell_{t-1}),
\]
where the transition matrix $P(\ell_{t+1} \mid \ell_t)$ is estimated from corpus statistics and a twenty-seventh probability mass at each state corresponds to a termination event. The generator is a finite stochastic automaton with an absorbing END state.

This structure produces outputs with genuine phonotactic plausibility. The learned transitions encode which letter sequences are statistically consistent with English pronunciation, so generated strings feel pronounceable even when they are not recognizable words. The generator trained on Shakespeare and English literature thus produces a specific distribution over $\Sigma^*$ that is strongly concentrated on phonologically coherent strings while still having positive probability mass across the entire string space.

The key structural observation is that this generator has full support over $\Sigma^*$ in the same sense that the Library of Babel contains all strings: for any string with no zero-probability transitions, the generator can produce it with nonzero probability. But the probability mass is overwhelmingly concentrated on a small region. More importantly, the generator defines no mapping from its output strings into any space of structured objects with internal invariants. Each generated string is a label---a phonologically shaped token---but it has no internal structure that could serve as the anchor for consistent semantic interpretation. It produces labels without objects.

\subsection{The First Experiment and Its Diagnostic Value}

The diagnostic value of the failure described in the opening section is precisely that it reveals the boundary between label generation and structured generation. When novel names generated by the Markov chain were presented to an early generative language model with the instruction to describe the philosophical or religious content implied by each name, every name received a description drawn from a small set of semantic attractors: non-theistic ethical traditions centered on harmony, personal growth, environmental stewardship, and compassion. The descriptions were plausible and internally coherent, but they were indistinguishable in structure across all thirty names regardless of phonological character.

The reason is not a limitation of the language model's intelligence but a structural consequence of the absence of constraint coupling. The language model interprets an unknown token by projecting it onto the nearest region of its learned semantic manifold. Since all the generated names were equally ungrounded---none had any internal structure distinguishing it from the others---they all projected onto the same high-probability region of the manifold: the cluster of concepts most frequently associated with novel philosophical or spiritual terminology in the training data. The model was doing exactly what it should do given the information available to it.

This failure is structurally distinct from a training data limitation or a prompting failure. No amount of prompt engineering could have produced genuinely distinct outputs, because there was nothing in the inputs to differentiate on. The constraint that would distinguish Klabans from Pfichrvetzrdah or Jolverslperowivicachmebunck would have to come from a structure defined prior to the name---from a parameter space encoding ontological commitments, epistemic orientations, social arrangements, and ritual structures---and that structure would then need to be projected into the name rather than extracted from it after the fact. This is the inversion the Substrate Theorem later captures: a symbolic substrate carries structure faithfully only when paired with a decoder that preserves the relevant distinctions, not when interpretation is delegated to a model operating on a decontextualized string.

\subsection{Spectral Structure of Generative Processes}

The transition matrix of a Markov generator is not merely a lookup table; it is an operator on distributions over the alphabet. Applying the operator repeatedly produces a trajectory through the simplex of probability distributions over letters, converging toward the stationary distribution of the chain. The eigenstructure of the transition matrix determines the rate of convergence and the shape of the stationary distribution, which in turn determines the statistical character of generated strings.

This algebraic perspective implies that modifying the transition matrix is not merely adding noise---it is changing the spectral structure of the generator. Temperature scaling of the transition probabilities, defined by $P_{ij}^{(T)} = P_{ij}^{1/T} / \sum_k P_{ik}^{1/T}$, sharpens or flattens the distribution while preserving its qualitative structure. As $T \to 0$ the generator becomes maximally deterministic; as $T \to \infty$ it approaches a uniform distribution over the alphabet. This is a continuous deformation of the generator within a fixed structural regime.

More radical modifications change the regime itself. Injecting noise directly into the matrix entries---replacing $P_{ij}$ with $P_{ij} + \epsilon_{ij}$ followed by renormalization---introduces incoherence into the transition structure, producing strings that feel like distorted rather than foreign language. Applying a structured modulation such as $P_{ij}^{(\mathrm{mod})} = P_{ij} \cdot (1 + \epsilon \sin(\omega \cdot f(i,j)))$ preserves the qualitative pattern while introducing a systematic periodic deformation, producing strings that feel consistently alien rather than randomly distorted. Rewiring the transition topology entirely---changing which transitions are possible rather than their probabilities---produces a genuinely different phonotactic regime, a different linguistic physics rather than a variant of the original.

The insight that a text or a generative process has a spectral signature extends to longer sequences and richer representations. Given any mapping from symbols to numerical values---whether raw Unicode codepoints, learned embeddings, or a custom encoding scheme---a text string becomes a finite signal, and its discrete Fourier transform reveals the frequency content of that signal. Concentration of spectral energy in a small number of frequencies indicates repetitive or rhythmically structured generation; a broad flat spectrum indicates high-entropy generation. Two texts with similar spectral signatures share generative regularities at the level of their encoded representation, even if their surface forms differ. This is what the Substrate Theorem formalizes as spectral preservation: a coherent encoding-decoding pair must preserve the spectral distinctions that separate generatively inequivalent histories.

\subsection{The Asymmetry Between Ecosystem Simulation and Self-Maintenance}

A critical distinction underlying the entire framework is the structural asymmetry between simulating an ecosystem and maintaining a self-narrative. These are not merely tasks of different difficulty; they belong to fundamentally different constraint regimes.

An ecosystem can be modeled as a dynamical system in which each state is a function of the previous state and local interactions. Formally, $x_{t+1} = F(x_t, \text{environment})$. Even when the system is complex and chaotic, it is externally describable and locally updateable. Constraints are local or statistical. Contradictions and inconsistencies can dissipate without structural consequence. The system does not need to maintain a coherent account of its own history. It does not need to be interpretable in hindsight. It does not need to preserve an identity across transformations. Simulating it correctly requires modeling the dynamics of interactions, not the coherence of accumulated commitments.

A self-narrative is a categorically different object. It is not a trajectory through state space but a history $H_t = (\omega_1, \omega_2, \dots, \omega_t)$ subject to a global consistency constraint $\mathcal{C}(H_t) = 1$. Every new event must be consistent with the entire prior history, not merely with the most recent state. This constraint is global, non-Markovian, and grows in complexity with $t$: each committed event reduces the admissible set of future events, and contradictions accumulate irreversibly. The system cannot simply restart or dissipate an inconsistency; it must account for it. This is what makes self-maintenance harder than ecosystem simulation by a structural argument rather than a complexity argument.

The further implication is that identity is not a property of a state but an invariant over histories. There exists an invariant $I$ such that $I(H_t) = I(H_{t-1})$ even as $H_t$ grows. The self must change while preserving the thing that defines it. No finite-state or locally-constrained system can enforce this property exactly, because the constraint is global and the history is unbounded. This is the core result that the non-finite-type boundary theorems later establish rigorously.

\subsection{Policy Selection as Constraint-Preserving Narrative Extension}

The connection between policy selection and narrative maintenance is that they are formally the same problem. A policy in the ordinary decision-theoretic sense maps states to actions: $\pi : x_t \to \omega_t$. But a policy for a self-maintaining system must map entire histories to actions: $\pi : H_t \to \omega_t$, subject to the constraint that $\mathcal{C}(H_{t+1}) = 1$. Each action is not merely evaluated for its immediate consequences but for its effects on the accumulated constraint structure and the admissible set of future actions.

This reframing has three consequences that the later theorems develop. First, every action is a projection that collapses part of possibility space: $\Omega(H_{t+1}) \subseteq \Omega(H_t)$. Good policies preserve more possibility space; bad policies collapse it toward contradictions or dead ends. Second, some actions expand possibility space by introducing new representational capacity, restructuring constraints, or adding new operators. These generative actions are structurally rare and expensive because they must remain coherent with accumulated history while genuinely enlarging what is expressible. Third, the optimal policy objective is not reward maximization but the preservation of the largest coherent future manifold consistent with identity: $\max_{\omega_t} |\Omega(H_{t-1} \cup \{\omega\}) \cap \mathcal{I}|$ where $\mathcal{I}$ is the set of identity-preserving trajectories.

The formalisms that follow---constraint functionals, admissibility filtrations, expansion operators, narrative type systems, history toposes---are the mathematical development of these three consequences. The Collapse Problem that opens the main argument, the Markov experiment, is the empirical observation that reveals all three in their absence: a generator without constraint coupling, operating in a flat possibility space, with no expansion operators and no identity to preserve, producing outputs that collapse to the prior distribution of whatever interprets them.
